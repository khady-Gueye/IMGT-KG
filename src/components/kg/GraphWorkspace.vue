...
<p class="tab-subtitle">
  {{ tabSubtitles[currentNav] || '' }}
        ...
        <div class="search-bar-center">
          <input
            type="text"
            v-model="query"
            placeholder="Mab Selection"
            @keyup.enter="search"
          />
          <button @click="search">Rechercher</button>
        </div>
        <GraphDisplay v-if="filteredResults.length && currentNav === 'imgt-mab-kg'" :triples="filteredResults" />
        <button
  ...
  async function search() {
    ...
    results.value = []
    try {
      const sparqlQuery = await renderQuery("../templates/query.rq", { query: query.value })
